...
contract Chess {
    ...
    */
	...
	function verifyExecuteKnightMove(
		...
		if (pieceToPosition > 0) {
			if (
				((pieceToPosition & color_const) == color_const) == currentTurnBlack
			) {
		...
		if (bitSize > piece_bit_size) {
			...
			if ((piece > 0) && ((piece & color_const) == color)) {
				...
				if (
					...
					checkKnightValidMoves(gameState, pos, playerState, currentTurnBlack)
				) {
					return true;
				} else if (
					...
					checkBishopValidMoves(gameState, pos, playerState, currentTurnBlack)
				) {
					return true;
				} else if (
